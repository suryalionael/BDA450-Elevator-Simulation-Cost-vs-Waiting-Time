...
------
* Time unit  : seconds.
* Floors     : 0 (parking) through 4 (top floor).
* Algorithm  : cooperative SCAN — every elevator sees every waiting
  passenger. No per-elevator call ownership. Any car that sweeps past
  a floor will stop for same-direction waiters it can board. This is
  more realistic and more stable than a dispatcher for a 5-floor building.

Why no dispatcher?
  In a small building, strict call ownership prevents free elevators from
  helping nearby passengers, collapses the benefit of extra cars, and
  creates long tails when an assigned elevator is delayed. Cooperative
  SCAN lets cars share the load naturally.

Key correctness points
----------------------
1. Direction-flip re-check: after reversing direction at any floor, the
   car immediately re-evaluates _should_stop() in the new direction.
   Without this, passengers at the reversal floor waiting in the new
   direction are left behind.

2. pass_count: incremented whenever a full car reaches a floor where
   same-direction passengers are waiting but cannot board. Used for
   floor-fairness / starvation analysis.

3. Idle wake-up: all idle elevators share one Building.wakeup event.
   Any passenger arrival signals the event so all waiting cars wake up.
"""
...
class Building:
    ...
    No dispatcher; all elevators read from the same queues.
    """
    ...
    def requests_ahead(self, floor: int, direction: int) -> bool:
        ...
        (either direction). The elevator must travel there to reach the
        farthest relevant request, and will serve opposite-direction waiters
        after reversing at that floor.
        """
...
class Elevator:
    ...
    Stopping rule
    -------------
    Stop at the current floor if:
      (a) an on-board passenger exits here, OR
      (b) same-direction waiters are present and the car is not full.
    After reversing direction, rule (b) is re-evaluated immediately so
    passengers at the turnaround floor in the new direction board at once.

    Fairness metric
    ---------------
    When the car arrives full at a floor where same-direction passengers
    are waiting, each stranded passenger gets pass_count += 1.
    """
    ...
    def _load(self) -> None:
        ...
        Any waiters left behind because the car filled up get pass_count += 1.
        """
...
def run_simulation(
    ...
    Passengers still waiting at sim_duration are silently dropped.
    """
    if not passengers:
        return []
...
def run_simulation_full(
    ...
    -------
    dict with keys:
        completed   – finished-trip passenger records
        unserved    – passengers still waiting at sim end
        n_total     – total passengers injected
