...
N_REPS          = 10          # replications per scenario (use 20 for report)
...
def build_summary_row(num_elevators: int, traffic_scale: float,
    ...
    return {
        ...
        # Waiting time
        "wait_mean_s":      round(wt["mean"], 1),
        "wait_std_s":       round(wt["std"],  1),
        "wait_p50_s":       round(wt["p50"],  1),
        "wait_p90_s":       round(wt["p90"],  1),
        "wait_p95_s":       round(wt["p95"],  1),
        "wait_max_s":       round(wt["max"],  1),
        # Total trip time
        "trip_mean_s":      round(st["mean"], 1),
        "trip_p90_s":       round(st["p90"],  1),
        "trip_p95_s":       round(st["p95"],  1),
        # Fairness: mean pass_count (how often full elevator passed someone)
...
def main():
    ...
    # ── Print per-floor fairness tables for baseline ─────────────────
    print(f"\n{'='*65}")
    ...
    # ── Print time-of-day tables for baseline ────────────────────────
    print(f"\n{'='*65}")
    ...
    # 1. Waiting-time histogram for each elevator count (baseline traffic)
    for elev in ELEVATOR_COUNTS:
        sub = all_records[(all_records["num_elevators"] == elev) &
                          (all_records["traffic_scale"] == 1.0)]
        plot_waiting_histogram(
    ...
    plot_boxplot_by_elevators(
        scale_dfs_wait, metric="waiting_time",
    ...
    # 4. Time-of-day comparison — for each elevator count at baseline
    for elev in ELEVATOR_COUNTS:
        sub = all_records[(all_records["num_elevators"] == elev) &
                          (all_records["traffic_scale"] == 1.0)]
        plot_tod_comparison(
    ...
    # 5. Waiting timeline over the day — for each elevator count at baseline
    for elev in ELEVATOR_COUNTS:
        sub = all_records[(all_records["num_elevators"] == elev) &
                          (all_records["traffic_scale"] == 1.0)]
        plot_waiting_over_time(
    ...
    # 6. Floor × time-of-day heatmap (reveals rush-hour starvation)
    for elev in ELEVATOR_COUNTS:
        sub = all_records[(all_records["num_elevators"] == elev) &
                          (all_records["traffic_scale"] == 1.0)]
        plot_rush_floor_heatmap(
    ...
    # 7. Cost vs. service quality (baseline traffic only)
    baseline_summary = summary[summary["traffic_scale"] == 1.0].copy()
    plot_cost_vs_service(
        summary,
