...
# ─────────────────────────────────────────────
# Time-of-day buckets — full 24-hour coverage
# Passengers outside 00:00–24:00 cannot exist,
# so "Other" should never appear in practice.
# ─────────────────────────────────────────────
TOD_BUCKETS = {
    "Early\n00–06":      ( 0 * 3600,  6 * 3600),
    "Morning\n06–09":    ( 6 * 3600,  9 * 3600),
...
def _tod_label(t: float) -> str:
    ...
    Times are normalised modulo 86 400 so a value of exactly 86 400 (i.e.
    midnight roll-over) maps to the Early bucket rather than "Other".
    """
    t = t % 86_400
...
def _tod_order_present(series: pd.Series) -> list:
    ...
    unlabelled values (e.g. the "Other" safety-net bucket).
    Guarantees no bucket is silently excluded from plots or tables.
    """
...
def records_to_df(records: list) -> pd.DataFrame:
    ...
    Columns added:
        ...
        time_of_day   – categorical TOD bucket label
        pass_count    – times a full elevator passed this passenger
    """
...
def stats_by_tod(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Uses full 24-hour bucket set; no passenger is excluded.
    """
    rows = []
...
def plot_tod_comparison(
    ...
    """
    Two-panel figure comparing waiting_time and system_time across all
    time-of-day buckets present in the data (full 24-hour coverage).
    No bucket is silently excluded — unlabelled passengers ("Other") are
    appended at the end if they exist.
    """
...
def plot_waiting_over_time(
    ...
    """
    Line chart of mean waiting_time in rolling bin_minutes-wide bins,
    with a shaded ±1 std band.  Rush-hour spikes are immediately visible.
    Covers the full day based on actual arrival times in the data.
    """
...
def plot_rush_floor_heatmap(
    ...
    """
    Heatmap: rows = origin floors, cols = all TOD buckets present in data.
    Uses _tod_order_present so no bucket is silently excluded.
...
def plot_cost_vs_service(
    ...
    Expects summary_df to have columns:
        num_elevators, traffic_scale, mean_wait_s
    (as produced by experiments.build_summary_row).
    """
    fig, ax = plt.subplots(figsize=(8, 5))
